...
group<-pData(gseineed)[['genotype/variation:ch1']]   
group_age<-pData(gseineed)[['age:ch1']]   #
group=c(rep("wt",6),rep("NLRP3_KO",6))  #Modify the name slightly in case of special group name
group_age <- c(rep("young",3),rep("old",3),rep("young",3),rep("old",3))#Shorten the group name
Group<-as.data.frame(group)
Group_age<-as.data.frame(group_age)
rownames(Group)<-colnames(exp)
rownames(Group_age)<-colnames(exp)
...
designtable_age<-model.matrix(~0+factor(group_age))   #[factor] function turn group into facotr
colnames(designtable_age)<-levels(factor(group_age))  #for age
rownames(designtable_age)<-colnames(exp)

# 3.contrast matrix
contrast.matrix=makeContrasts(NLRP3_KO - wt,levels=designtable)
...
fit2<-eBayes(fit2) 
options(digits=4)
DEG<-topTable(fit2,coef="NLRP3_KO - wt",n=Inf)#coef should be the group name
...
retu <- pheatmap(df,
                 ...
                 main ="wt-NLRP3_KO",show_colnames = T,show_rownames = T,
                 legend_breaks = c(9,5,1),legend_labels = c("high","median","low"),
...
#We choose Uqcr gene as one example.

UqcrqGene <- jiyinbiaodaliangbiao[rownames(jiyinbiaodaliangbiao)==" Uqcrq ",] #find gene expression in Gene Expression Frame
      
plotdraft <- merge(Group,t(UqcrqGene), by = 'row.names', all = TRUE) 
plotdraft <- plotdraft[,c(2,3)]   #make gene expression frame of groups
dataset <- data.frame(Uqcrq <- plotdraft[,2], group <- factor(plotdraft[,1])) #transfer into factor
UqcrqBoxplot <- boxplot( Uqcrq ~ group , data = plotdraft, xlab = " ",
        ylab = " ", main = "Uqcrq")
